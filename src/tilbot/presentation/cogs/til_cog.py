...
class TilCog(commands.Cog):
    ...
    @commands.Cog.listener() #Cog内でイベントハンドラーを定義するためのデコレーター。on_readyイベントは、BotがDiscordに接続して準備ができたときに呼び出されるイベント
    async def on_message(self, message: discord.Message) ->None:
        if message.author.bot: #Bot自身のメッセージや他のBotのメッセージを無視するためのチェック。これにより、Botが自分自身のメッセージに反応して無限ループになることを防ぐことができる。
            return
        
        if message.channel.id != self.config.til_channel_id: #特定のチャンネルでのみ反応するためのチェック。これにより、Botが他のチャンネルのメッセージに反応しないようにすることができる。
            return
